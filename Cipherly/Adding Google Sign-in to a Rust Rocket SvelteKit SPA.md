...
2. And the server fetches Google's public keys.
...
2. The server validates the token using Google's public keys.
...
The payload contains a set of claims from Google's auth server. If the token's signature is valid, then we can trust that the claims have not been tampered with. I've tampered with this JWT to hide my personally identifiable information, so if we scroll down, we can that the signature is invalid.

## Google Certs

Tokens that use the RS256 algorithm, are signed using the private key of a public/private key pair. We can validate the token and prove that the token was not tampered with if we have the public key. For now, this is all you need to know, but if you're especially curious, computerphile has done an excellent primer. They should show up as card somewhere around here!

```
https://www.googleapis.com/oauth2/v3/certs
```

So, in order to validate the token, we need Google's public key. We can get it at this URL. In this case, there are two public keys. The one that was used to sign our JWT is the one where the `kid` matches the token's `kid`.

Our Rocket server needs to get these keys. So, in Rust, we have a fetch function that uses the reqwest library to make a call to the Google public keys URL. This function is called at startup and these keys are made available to every call to our server.
...
So here are the validation steps. First, we get Google's public keys. Then, we extract the Authorization header from the request. If that fails, we return unauthorized. Then we extract the bearer token, parse the header, and get the `kid` that signed the token. If anything fails, unauthorized. Once we have the `kid`, we can get the corresponding Key from Google's public keys. Then we call decode, which validates the bearer token using Google's public key, as well as a list of expected claims. In our case, we ensure that the algorithm is RS256, the audience is this thing, and the issuer is Google. If that succeeds, we return the decoded claims, which for our server are the user's email, name, and token expiration time.
...
If you liked this video, hit the thumbs up. I'm The FriendlyTL and I make videos about useful open source apps that I also make such as Cipherly, CodeAnim, and Hexly. If you're interested in the apps that I maintain, and the new apps that I'll be making in the future, subscribe! Thanks for watching; I'll see you in the next one!
